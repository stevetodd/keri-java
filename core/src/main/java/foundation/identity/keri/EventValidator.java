...
public final class EventValidator {
  ...
  public void validate(IdentifierState state, IdentifierEvent event) {
    if (!(event instanceof InceptionEvent)) {
      requireNonNull(state, "state is required for non-inception events");

      // prefix
      require(state.identifier().equals(event.identifier()), "state and event prefix must match");

      // sequence number
      require(event.sequenceNumber().compareTo(BigInteger.ZERO) > 0,
          "non-inception events must have a sequence number greater than 0 (given: %s)", event.sequenceNumber());
      ...
      // previous (digest)
      require(matchingDigests(state.lastEvent(), event.previous()),
    ...
    if (event instanceof EstablishmentEvent) {
      ...
      // signature
      requireValidEventSignatures(state, event);
